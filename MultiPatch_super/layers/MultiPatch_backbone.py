...
from layers.MultiPatch_layers import *
...
class MultiPatch_backbone(nn.Module):
    def __init__(self, c_in:int, context_window:int, target_window:int, patch_len:str, stride:int, max_seq_len:Optional[int]=1024,
        ...
        self.patch_lens = []
        for patch_index, p_len in enumerate(self.patch_len):
            p_len = int(p_len)
            self.patch_lens.append(p_len)
            patch_num = int((context_window - p_len)/stride + 1)
            ...
            if self.pretrain_head:
                self.heads.append(self.create_pretrain_head(head_nf, c_in, fc_dropout)) # custom head passed as a partial func with all its kwargs
    ...
    def forward(self, z):                                                                   # z: [bs x nvars x seq_len]
        ...
        for i, p_len in enumerate(self.patch_lens):
            ...
            # model
            x = self.backbones[i](x)                                                                # z: [bs x nvars x d_model x patch_num]
            x = self.heads[i](x)                                                                    # z: [bs x nvars x target_window]
